...
export default function DiarioPage() {
  ...
  const { data: resultado, isLoading } = useQuery({
    queryKey: ["diario", data],
    queryFn: () => contabilidadeService.diario(data),
  });
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-ink dark:text-white">Diário</h1>
      </div>
      ...
      <div className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-800 dark:bg-amber-950/40">
        ...
        <p className="text-sm text-amber-700 dark:text-amber-300">Fonte: dados internos — lista cronológica de lançamentos do dia.</p>
      </div>

      <input type="date" value={data} onChange={(e) => setData(e.target.value)} className="px-3 py-2 border rounded-lg dark:bg-ink-ghost/20 dark:border-ink-ghost/20" />
      ...
      <div className="bg-panel dark:bg-panel rounded-xl shadow overflow-hidden">
        <table className="w-full">
          <thead className="bg-surface dark:bg-ink-ghost/20"><tr className="text-left text-xs uppercase text-ink-mid/70">
            <th className="px-4 py-3">Hora</th><th className="px-4 py-3">Tipo</th><th className="px-4 py-3 text-right">Valor</th>
          </tr></thead>
          <tbody className="divide-y divide-ink-ghost/40 dark:divide-ink-ghost/15">
            ...
            {!isLoading && (resultado?.lancamentos.length ?? 0) === 0 && <tr><td colSpan={3} className="p-8 text-center text-ink-mid/70">Sem lançamentos neste dia</td></tr>}
            {resultado?.lancamentos.map((l: any, i: number) => (
              <tr key={i}>
                <td className="px-4 py-3 text-sm">{new Date(l.data).toLocaleTimeString("pt-PT")}</td>
