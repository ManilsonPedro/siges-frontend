...
export default function FiscalizacaoPage() {
  ...
  const { data: vendas = [], isLoading } = useQuery({
    ...
    queryFn: () => caixaService.listVendas({ pendente_faturacao: true, page_size: 200 }),
  });
  ...
  const marcarMut = useMutation({
    mutationFn: ({ id, ref }: { id: string; ref: string }) => caixaService.marcarFaturada(id, ref),
    onSuccess: () => { toast.success("Marcada como faturada"); qc.invalidateQueries({ queryKey: ["caixa"] }); },
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <p className="text-sm text-ink-mid dark:text-gray-400">
        Vendas concluídas que ainda não têm nº de fatura interna atribuído. Após emitir a fatura,
        cole aqui o nº para fechar o ciclo.
      </p>
